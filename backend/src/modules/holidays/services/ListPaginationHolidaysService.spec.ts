...
import ListPaginationHolidaysService from './ListPaginationHolidaysService';

let draftHolidaysRepository: DraftHolidaysRepository;

let draftCacheProvider: DraftCacheProvider;

let listPaginationHolidays: ListPaginationHolidaysService;

describe('ListPaginationHolidays', () => {
  beforeEach(() => {
    draftHolidaysRepository = new DraftHolidaysRepository();
    draftCacheProvider = new DraftCacheProvider();

    listPaginationHolidays = new ListPaginationHolidaysService(
  ...
  it('should not be able to list holidays without authentication', async () => {
    ...
    await expect(
      listPaginationHolidays.execute('', 1, null),
  ...
  it('should be able to list all the holidays from the first page', async () => {
    ...
    const response = await listPaginationHolidays.execute('', 1, user_id);
  ...
  it('should not be able to list holidays from the second page without accumulate the first one', async () => {
    ...
    const response = await listPaginationHolidays.execute('', 2, user_id);
  ...
  it('should be able to return the same holidays if the same request is made', async () => {
    ...
    await listPaginationHolidays.execute('', 1, user_id);
  ...
  it('should be able to list all the holidays from the first page which includes a search string', async () => {
    ...
    const response = await listPaginationHolidays.execute(
      holidaySearch,
      1,
      user_id,
    );
